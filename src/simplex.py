...
from display import print_initial, print_tableau
...
def canonical_form(lp):
    ...
    # Add slack variables
    objp = np.zeros(n + m, dtype=np.int64) + Fraction()
    objp[0:n] = obj

    Ap = np.zeros((m, n + m), dtype=np.int64) + Fraction()
    Ap[0:m, 0:n] = A
    for i in range(m):
        Ap[i, n + i] = 1
...
def prepare_phase_i(lp):
    ...
    if len(additonal_variables) == 0:
        return None
    # Add slack variables
    Ap = np.zeros((m, n + len(additonal_variables)), dtype=np.int64) + Fraction()
    Ap[0:m, 0:n] = A
    bp = np.array(b)
    # Set objective function to -Sum u_i
    objp = np.zeros(n + len(additonal_variables), dtype=np.int64) + Fraction()
    objv = Fraction(0, 1)
...
def after_phase_i(lp, basis, t):
    ...
    objv_r = Fraction(0)
    obj_r, A_r, b_r = np.zeros(n, dtype=np.int64) + Fraction(), np.array(A), np.array(b)
    obj_r[: len(obj)] = obj

    for i in range(t, n):
        A_r[:, i] = np.zeros(m, dtype=np.int64) + Fraction()
...
def simplex_solver(lp, objv, basis, rule, verbose, phaseI, forbidden=[]):
    ...
    outside = list(filter(lambda x: x > -1, [i if i not in basis else -1 for i in range(n)]))
...
def simplex(lp, rule, verbose, rule_name):
    ...
    if res:
        ...
        objv2, basis2, (_, A2, b2), _, c1 = simplex_solver(
            (obj1, A1, b1), objv1, basis1, rule, verbose, True
        )
        if verbose:
            print("\nPhase I is over\n")
        if objv2 < 0:
    ...
    # phase II
    objv, basis_res, (_, A_res, b_res), status, c2 = simplex_solver(
        (obj3, A3, b3), objv3, basis3, rule, verbose, False
    )
    if status == "Unbounded":
        ...
        print(
            ...
            + ", ".join(list(map(lambda x: "x_" + str(x[0]) + " = " + str(x[1]), opt)))
        )
